...
# 【已解决】Windows 11 精简版更新报错 `0x80073712`：DISM 卡死、sfc 无法启动、WinRE 被删（组件存储损坏 · UUP 镜像就地升级修复）
...
**关键词**：`0x80073712` · `0x800f0915` · `ERROR_SXS_COMPONENT_STORE_CORRUPT` · 组件存储损坏 4114 处 · DISM `/RestoreHealth` 卡死不动 · `sfc /scannow` 报 source file in store is also corrupted · WinRE / `winre.wim` 被删 · 精简版 Windows 11 无法更新 / 更新页面打不开 · `LCUInstallErrorPackageNames` · UUP dump 定制镜像 · In-Place Upgrade 就地修复升级 · setup 拒绝降级（ISO 基线低于本机 UBR）
